...
struct ResponseInformation
{
    ...
    ResponseProperties mapped_properties;

    ResponseInformation(CommandPayloadInfo type_info, ResponseProperties properties)
        : payload_info(type_info), mapped_properties(properties) {}

    size_t get_num_properties() const { return mapped_properties.size(); }
};
...
const std::map<CommandIds, ResponseInformation> g_response_payload_map = {
    ...
    {RUN_MOTOR_ABSOLUTE_MOTION_BY_AXIS, ResponseInformation(CommandPayloadInfo(PayloadType::UINT8), ResponseProperties{std::any(&MotorProperties::target_position)})},   // Speed, acceleration, and absolute axis (uint24)
    {RUN_MOTOR_RELATIVE_MOTION_BY_AXIS, ResponseInformation(CommandPayloadInfo(PayloadType::UINT8), ResponseProperties{std::any(&MotorProperties::target_position)})},   // Speed, acceleration, and relative axis (uint24)
    {RUN_MOTOR_RELATIVE_MOTION_BY_PULSES, ResponseInformation(CommandPayloadInfo(PayloadType::UINT8), ResponseProperties{std::any(&MotorProperties::target_position)})}, // Speed, acceleration, relative pulses (uint24)
    {RUN_MOTOR_ABSOLUTE_MOTION_BY_PULSES, ResponseInformation(CommandPayloadInfo(PayloadType::UINT8), ResponseProperties{std::any(&MotorProperties::target_position)})}, // Speed, acceleration, absolute pulses (uint24)
